...
class SaltDataset(Dataset):
    def __init__(self, transform, mode, image_size, fold_index, aug_list, pseudo_csv = None, pseudo_index = -1):
        ...
        # change to your path
        self.train_image_path = r'../../tgs-dataset/train/images'
        self.train_mask_path = r'../../tgs-dataset/train/masks'
        self.test_image_path = r'/home/gjt/datasets/Kaggle/KGS/test/images'
    ...
    def __getitem__(self, index):
        ...
        border_label = cv2.Canny(label.reshape([self.image_size, self.image_size]), 50, 150)
        border_label = np.asarray(border_label).astype(np.float32) / 255.0
        border_label[border_label >= 0.5] = 1.0
        border_label[border_label < 0.5] = 0.0

        label = np.asarray(label).astype(np.float32) / 255.0
        label[label >= 0.5] = 1.0
        label[label < 0.5] = 0.0

        return torch.FloatTensor(image), torch.FloatTensor(label), is_empty, torch.FloatTensor(border_label)
