...
from model.ibnnet import resnext101_ibn_a,IBN
...
class model34_DeepSupervion(nn.Module):
    def __init__(self, num_classes=1, mask_class = 2):
        ...
        self.e2se = SCSEBlock(64)
        self.e3se = SCSEBlock(128)
        self.e4se = SCSEBlock(256)
        self.e5se = SCSEBlock(512)

        self.logits_no_empty1 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, padding=1),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(32, 1, kernel_size=1, padding=0))

        self.logits_no_empty2 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, padding=1),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(32, 1, kernel_size=1, padding=0))
        self.logits_no_empty3 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, padding=1),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(32, 1, kernel_size=1, padding=0))
        self.logits_no_empty4 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, padding=1),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(32, 1, kernel_size=1, padding=0))

        ## this is for boarder net work
        self.rrb_db_1 = RRB(64, 64)
        self.rrb_db_2 = RRB(128, 64)
        self.rrb_db_3 = RRB(256, 64)
        self.rrb_db_4 = RRB(512, 64)

        self.rrb_trans_1 = RRB(64, 64)
        self.rrb_trans_2 = RRB(64, 64)
        self.rrb_trans_3 = RRB(64, 64)

        self.rrb_trans_4 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, padding=1),
                                         nn.ReLU(inplace=True),
                                         nn.Conv2d(32, 1, kernel_size=1, padding=0))
        ...
        # self.decoder = Decoder(64, 32, 64)

        # self.border_feat_expand = nn.Sequential(nn.Conv2d(64, 320, kernel_size=1, padding=0), nn.ReLU(inplace=True))

    def forward(self, x): #(batchsize，channels，h，w):[16,3,128,128]
        conv1 = self.conv1(x)     #1/4 [16,64,64,64]
        conv2 = self.e2se(self.conv2(conv1)) #1/4 [16,64,64,64]
        conv3 = self.e3se(self.conv3(conv2)) #1/8 [16,128,32,32]
        conv4 = self.e4se(self.conv4(conv3)) #1/16 [16,256,16,16]
        conv5 = self.e5se(self.conv5(conv4)) #1/32 [16,512,8,8]

        # for border network
        res1 = self.rrb_db_1(conv2)
        res1 = self.rrb_trans_1(res1 + self.upsample(self.rrb_db_2(conv3)))
        res1 = self.rrb_trans_2(res1 + self.upsample_4(self.rrb_db_3(conv4)))
        border_fm = F.upsample((self.rrb_trans_3(res1 + self.upsample_8(self.rrb_db_4(conv5)))), scale_factor=2,mode='bilinear')
        x_border = self.rrb_trans_4(border_fm)

        center_512 = self.center_global_pool(conv5) #[16,512,1,1]
        center_64 = self.center_conv1x1(center_512) #[16,64,1,1]
        center_64_flatten = center_64.view(center_64.size(0), -1)#[16,64] view()函数的功能根reshape类似，用来转换size大小。x = x.view(batchsize, -1)中batchsize指转换后有几行，而-1指在不告诉函数有多少列的情况下，根据原tensor数据和batchsize自动分配列数。
        center_fc = self.center_fc(center_64_flatten) #[16,2]

        f = self.center(conv5) #[16,256,4,4]
        d5 = self.decoder5(f, conv5) #[16,64,8,8]
        d4 = self.decoder4(d5, conv4)#[16,64,16,16]
        d3 = self.decoder3(d4, conv3)#[16,64,32,32]
        d2 = self.decoder2(d3, conv2)#[16,64,64,64]
        ...
        hypercol = torch.cat((
            ...
            F.upsample(d5, scale_factor=16, mode='bilinear')),1)#[16,320

        # hypercol = torch.add(hypercol,self.border_feat_expand(border_fm))

        x_no_empty = self.logits_no_empty(hypercol)#[16,1,128,128]
        x_no_empty1 = self.logits_no_empty1(d1)
        x_no_empty2 = F.upsample(self.logits_no_empty2(d2), scale_factor=2, mode='bilinear')
        x_no_empty3 = F.upsample(self.logits_no_empty3(d3), scale_factor=4, mode='bilinear')
        x_no_empty4 = F.upsample(self.logits_no_empty4(d4), scale_factor=8, mode='bilinear')


        hypercol_add_center = torch.cat((
            hypercol,
            F.upsample(center_64, scale_factor=128,mode='bilinear')),1)#[16,384,128,128]

        x_final = self.logits_final( hypercol_add_center)#[16,1,128,128]
        return center_fc, [x_no_empty,x_no_empty1,x_no_empty2,x_no_empty3,x_no_empty4], x_final, x_border
...
from model.senet import se_resnext101_32x4d
...
from model.senet import se_resnet152
